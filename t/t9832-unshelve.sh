...
EOF
	...
	(
		...
		git show refs/heads/p4/unshelved/$change | grep -q "Further description" &&
		git cherry-pick refs/heads/p4/unshelved/$change &&
...
EOF
	...
	(
		...
		change=$(last_shelved_change) &&
		git p4 unshelve $change &&
		git diff refs/heads/p4/unshelved/$change.0 refs/heads/p4/unshelved/$change | grep -q file3
	)
'
